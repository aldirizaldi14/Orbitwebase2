...
class ItemStaticController extends BaseController
{
    ...
    public function data(Request $request)
    {
        ...
        $filter = DB::raw("(
            ...
            OR LOWER(item_desc) LIKE '%".strtolower($filter)."%'
            
        )");

        //$data = ItemStaticModel::orderBy($sort, $dir)->whereRaw($filter)->where('spq', $request->user()->user_side);
        $data = ItemStaticModel::orderBy($sort, $dir)->whereRaw($filter);

        if ($length) {
            $data->skip($start)->take($length);
        }
        $data = $data->get();
        $count = ItemStaticModel::whereRaw($filter)->count();
