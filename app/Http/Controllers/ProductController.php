...
use App\Model\Product_ViewModel;
use Session;
use Storage;
use DB;
use Response;
...
class ProductController extends BaseController
{
    ...
    public function data(Request $request)
    {
        ...
        $data = Product_ViewModel::orderBy($sort, $dir)->whereRaw($filter)->where('product_side_alt', $request->user()->user_side);
        if ($length) {
            $data->skip($start)->take($length);
        }
        $data = $data->get();
        $count = Product_ViewModel::whereRaw($filter)->count();
